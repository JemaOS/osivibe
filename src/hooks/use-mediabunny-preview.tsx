    ...
    const initClips = async () => {
      ...
      clipsRef.current.forEach((clip, id) => {
        if (!currentClips.has(id)) {
          ...
          if (clip.lastSample) {
            try { clip.lastSample.close(); } catch(e) {}
          }
          // Close buffer
          clip.buffer.forEach(item => {
            try { item.sample.close(); } catch(e) {}
          });
          // clip.input.close(); // If close method exists
          // clip.sink.close();
        }
      });
      ...
      try {
        ...
        if (bestBufferSample) {
          ...
          if (mbClip.lastSample && mbClip.lastSample !== bestBufferSample) {
             try { mbClip.lastSample.close(); } catch(e) {}
          }
          ...
          for (let k=0; k<used.length-1; k++) {
             if (used[k].sample !== bestBufferSample) {
                try { used[k].sample.close(); } catch(e) {}
             }
          }
